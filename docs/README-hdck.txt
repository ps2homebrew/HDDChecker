...
The HardDisk ChecKer (HDCK) module is an IOP module for ensuring the integrity of the APA scheme, which SONY uses to organize space on the PlayStation 2 HDD unit.
...
HDCK will do the following as checks:
    ...
    3. Check partition links for integrity issues, both forward and backward:
        a) Forward direction:
    ...
    5. If the I/O error persists (either because it cannot be fixed or if the partition is not a sub-partition of another partition),
