...
After successful initialization, FSCK will create an fsck device, which will have the following I/O functions:
    open:
...
FSCK will check the following:
    ...
    2. Root directory    - the integrity of inodes.
    3. Check all files    - the integrity of directory entries and inodes.
    ...
    6. If errors were fixed, bit 2 of the superblock is set.
...
The different FSCK versions:
    ...
    The FSCK module from the US Utility Discs is exactly the same. However, compared to the original Japanese version (despite the similar version number):
