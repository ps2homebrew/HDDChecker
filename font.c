...
static struct FontGlyphSlot *AtlasAlloc(Font_t *font, struct FontAtlas *atlas, short int width, short int height)
{
    ...
    GlyphSlot = NULL;
    if (atlas->buffer == NULL) { // No frontier (empty atlas)
        ...
        switch (wchar) {
            ...
            default:
                ...
                if (DrawGlyph(gsGlobal, &GS_FTFont, wchar, x, y, z, scale, colour, 0, &width) != 0) {
                    if (DrawGlyph(gsGlobal, &GS_sub_FTFont, wchar, x, y, z, scale, colour, 0, &width) != 0) { // Cannot locate the glyph, so draw the missing glyph character.
